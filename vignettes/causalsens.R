...
## @knitr loaddata, include=TRUE, cache=FALSE, tidy=FALSE
library(causalsens)
data(lalonde.exp)


## @knitr outcomemodel, tidy=FALSE
ymodel <- lm(re78 ~ treat+age + education + black + hispanic + married +
             nodegree + re74 + re75 + u74 + u75, data = lalonde.exp)
summary(ymodel)


## @knitr pscoremodel, tidy=FALSE
...
## @knitr sensplot,echo=2:3,eval=FALSE
## par(mfrow=c(1,2))
## plot(ll.sens, type = "raw", bty = "n")
## plot(ll.sens, type = "r.squared", bty = "n")


## @knitr sensplot, fig.width=10, fig.height=5, fig.cap="Results from LaLonde (1986)",echo=FALSE
par(mfrow=c(1,2))
plot(ll.sens, type = "raw", bty = "n")
plot(ll.sens, type = "r.squared", bty = "n")
